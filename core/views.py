...
def all_pronos_view(request):
    ...
    # 3. Détermination de la saison (FILTRÃ‰E par la compétition choisie)
    seasons = Season.objects.filter(competition=selected_comp, year__gte=2025).order_by('-year')
    ...
    # 4. Détermination des journées (FILTRÃ‰ES par la saison choisie)
    rounds = Round.objects.filter(season=selected_season).order_by('number')
...
def statistiques_view(request):
    ...
    for s in seasons_qs:
        if not competition:
            # Mode global : regrouper par clé de saison (ex: Top14 2025/2026 + 6N 2026 â†' "2025-2026")
...
def recap_pronos_classement(request):
    ...
    if competition_id:
        ...
        # Préparation des top3 réels (marqueur/réalisateur) + flags de réussite par joueur
...
def admin_saisie_resultats(request):
    ...
    if request.method == "POST":
        ...
        # 1. Sauvegarde des bonus réels (top 3, ex-aequo gérés via virgules)
...
def _season_label(season_year):
    """Libellé d'une saison archivée. Convention SeasonHistory : `season_year`
    = année de FIN de saison (2024-2025 est stocké 2025) → libellé 2024-2025."""
...
def hall_of_fame_view(request):
    ...
    # classement officiel « tout compris » archivé via l'admin.
...
def home_view(request):
    ...
    # --- 3. STATS GÃ‰NÃ‰RALES (Classement général) ---
    ...
    for r in relevant_rounds:
        ...
        if s_bo_p > 0 or s_bd_p > 0:
            debug_log.append(f"Round {str(r)} : BO {s_bo_ok}/{s_bo_p} | BD {s_bd_ok}/{s_bd_p}")
